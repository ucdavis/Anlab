...
describe("<IntegerInput />", () => {
    ...
    it("should clear error on good value", () => {
        ...
        const target = mount<IntegerInput>(<IntegerInput min={10} max={20} onChange={onChange} />);
    ...
    it("should set error on non-number value", () => {
        const onChange = jasmine.createSpy('onChange');
        const target = mount<IntegerInput>(<IntegerInput onChange={onChange}/>);
        const internal = target.instance();
    ...
    it("should set error on less than min value", () => {
        const onChange = jasmine.createSpy('onChange');
        const target = mount<IntegerInput>(<IntegerInput min={10} onChange={onChange}/>);
        const internal = target.instance();
    ...
    it("should set error on more than max value", () => {
        const onChange = jasmine.createSpy('onChange');
        const target = mount<IntegerInput>(<IntegerInput max={10} onChange={onChange}/>);
        const internal = target.instance();
