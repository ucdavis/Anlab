...
export class AdditionalEmails extends React.Component<
  IAdditionalEmailsProps,
  IAdditionalEmailsState
> {
  constructor(props) {
    super(props);
    this.state = {
    ...
    return this.props.addedEmails.map((item) => {
      return (
        <Badge key={item}>
          {item}
          <i
            className="emailIconStyle fa fa-times-circle"
            aria-hidden="true"
  ...
  private _renderError = () => {
    ...
    return (
      <span className="text-danger emailError">{this.state.errorText}</span>
    );
  };
  ...
  private _addEmailAddress = () => {
    ...
    if (
      emailtoAdd === this.props.defaultEmail ||
      this.props.addedEmails.indexOf(emailtoAdd) >= 0
    ) {
      this.setState({
